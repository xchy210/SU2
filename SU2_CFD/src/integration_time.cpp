    ...
    case RUNTIME_TNE2_SYS:
        ...
        if (config->GetConvCriteria() == RESIDUAL) {
          if (config->GetResidual_Func_Flow() == RHO_RESIDUAL) (*monitor) = log10(solver_container[FinestMesh][TNE2_SOL]->GetRes_RMS(0));
            ...
            // DELETE ME......THIS ISNT GENERAL, NEEDS TO BE FIXED
            if (nDim == 2) (*monitor) = log10(solver_container[FinestMesh][TNE2_SOL]->GetRes_RMS(4));
            else (*monitor) = log10(solver_container[FinestMesh][TNE2_SOL]->GetRes_RMS(5));
          }
        }
