...
void CTNE2EulerSolver::SetGradient_L2Proj2(CGeometry *geometry, CConfig *config){
  ...
  unsigned short iNode, iVar, iDim, iFlux, iSpecies;
  unsigned short nVarMetr = nVar, nFluxMetr = 2;  //--- TODO: adjust size of grad vector later for goal vs. feature
  su2double densityS[nSpecies], density, velocity[2], pressure, enthalpy, eve;
  ...
  for (iElem=0; iElem<nElem; ++iElem) {
    for (iNode=0; iNode<3; ++iNode) {
      ...
      for (iSpecies = 0; iSpecies < nSpecies; iSpecies++){
        densityS[iSpecies] = node[kNode]->GetPrimVar(iSpecies);
      }
      velocity[0] = node[kNode]->GetPrimVar(nSpecies+nDim+0);
      velocity[1] = node[kNode]->GetPrimVar(nSpecies+nDim+1);
      pressure    = node[kNode]->GetPrimVar(nSpecies+nDim+2);
      density     = node[kNode]->GetPrimVar(nSpecies+nDim+3);
      enthalpy    = node[kNode]->GetPrimVar(nSpecies+nDim+4);
      eve         = node[kNode]->GetSolution(nVar-1);
...
void CTNE2EulerSolver::SetHessian_L2Proj2(CGeometry *geometry, CConfig *config){
  ...
  unsigned short iNode, iVar, iDim, iFlux;
  unsigned short nVarMetr = nVar, nFluxMetr = 2;  //--- TODO: adjust size of grad vector later for goal vs. feature
