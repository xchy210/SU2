...
public:
  ...
  /*!
   * \brief virtual member that would be different for each gas model implemented
   * \param[in]  nEntities - Number of entities for which the pressure must
                             be computed.
   * \param[in]  rho       - Density for all entities.
   * \param[in]  e         - Internal energy per unit mass for all entities.
   * \param[out] p         - Pressure for all entities.
   */
  virtual void GetPressure_from_rhoe(const int       nEntities,
                                     const su2double *rho,
                                     const su2double *e,
                                     su2double       *p);
  ...
  /*!
   * \brief virtual member that would be different for each gas model implemented
   * \param[in]  nEntities - Number of entities for which the temperature must
                             be computed.
   * \param[in]  rho       - Density for all entities.
   * \param[in]  e         - Internal energy per unit mass for all entities.
   * \param[out] T         - Pressure for all entities.
   */
  virtual void GetTemperature_from_rhoe(const int       nEntities,
                                        const su2double *rho,
                                        const su2double *e,
                                        su2double       *T);

  /*!
   * \brief Function, which computes the laminar viscosity from the density and
            internal energy per unit mass for the given number of entities.
   * \param[in]  nEntities - Number of entities for which the pressure must
                             be computed.
   * \param[in]  rho       - Density for all entities.
   * \param[in]  e         - Internal energy per unit mass for all entities.
   * \param[out] muLam     - Laminar viscosity for all entities.
   */
  void GetLaminarViscosity_from_rhoe(const int       nEntities,
                                     const su2double *rho,
                                     const su2double *e,
                                     su2double       *muLam);
};
...
public:
  ...
  /*!
   * \brief Function, which computes the pressure from the density and
            internal energy for the given number of entities.
   * \param[in]  nEntities - Number of entities for which the pressure must
                             be computed.
   * \param[in]  rho       - Density for all entities.
   * \param[in]  e         - Internal energy per unit mass for all entities.
   * \param[out] p         - Pressure for all entities.
   */
  void GetPressure_from_rhoe(const int       nEntities,
                             const su2double *rho,
                             const su2double *e,
                             su2double       *p);

  /*!
   * \brief Function, which computes the temperature from the density and
            internal energy for the given number of entities.
   * \param[in]  nEntities - Number of entities for which the pressure must
                             be computed.
   * \param[in]  rho       - Density for all entities.
   * \param[in]  e         - Internal energy per unit mass for all entities.
   * \param[out] T         - Temperature for all entities.
   */
  void GetTemperature_from_rhoe(const int       nEntities,
                                const su2double *rho,
                                const su2double *e,
                                su2double       *T);
};
